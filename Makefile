...
synth-natifylambda: clean-cdk-out setup-cdk
	...
	@if sed -n '58p' cdk/natify_lambda_stack.py | grep -q 'code=lambda_.S3Code(bucket=s3_bucket, key=f"natifylambda-{natifylambda_version}.zip")'; then \
		echo "Proceeding with generation"; \
	else \
		echo "Line 58 does not contain the required string. Generation halted." && exit 1; \
	fi
	@npx cdk synth --quiet
...
synth: synth-natifylambda
	...
	@sed -i '' '58s/^#//' cdk/natify_lambda_stack.py
	@sed -i '' '60s/^/#/' cdk/natify_lambda_stack.py
